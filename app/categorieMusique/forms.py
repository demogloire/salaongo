...
class AjoutCatForm(FlaskForm):
    nom= StringField('Nom du type de musique', validators=[DataRequired("Completer nom"),  Length(min=4, max=32, message="Veuillez respecté les caractères")])
    submit = SubmitField('Categorieplaylist')

    #Fornction de verification d'unique existenace dans la base des données
    def validate_nom(self, nom):
        type= Categorieplaylist.query.filter_by(nom=nom.data).first()
        if type:
